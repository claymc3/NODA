...
####----------------------------------------------------------------------------------------------####
    ...
    for DataSet in MasterList:
        ...
        for file in InputFiles:
            ...
            in_df = pd.read_csv(file,sep='\s+',names=open(file).readline().rstrip().replace('Data Height', "Intensity").replace('Volume', "Volume  fit").replace('(hz)', "").split(),skiprows=[0,1],engine='python')
...
####----------------------------------------------------------------------------------------------####
    ...
    Title_label = {'N' : "Backbone",'H' : "Backbone", 'CMe': r"Methyl", "HMe": r"Methyl", "Me": "Methyl Intensity", "NE1":"W-NE1", "HE1":"W-HE1",
                   "Nside":'Side Chain N', "Hside":'Side Chain H', 'HB':'Side Chain HB2/HB3', 'HA':'Side Chain HA2/HA3', 'HG':'Side Chain HG2/HG3','CB':'Side Chain CB', 'CA':'Side Chain CA', 'CG':'Side Chain CG', 'CAro':'F/W CE1/CE2', 'CDAro':'F-CD1/2 Y-CD1/2 H-CD2','HDAro': 'F-HD1/2 Y-HD1/2 H-HD2', 'HEAro':'F-HE1/2 Y-HE1/2 H-HE1'}
    Plot_Dict = {}
    for dtype in Data_Types:
        ...
        for nuc in df['nuc'].unique().tolist():
            ...
            if dtype == 'CS':
                Plot_Dict[nuc + dtype]=[[vmin, vmax], r"$\Delta\delta$"+ Delta_ppm_dict[nuc]]
            elif dtype == 'CS_wavg':
                Plot_Dict[nuc + dtype]=[[0.0, vmax], Delta_wavg_dict[nuc]]
            elif dtype == 'LW':
                Plot_Dict[nuc + dtype]=[[0.0, vmax], r"LW/LW$_{0}$ "]
            elif dtype == 'Intensity':
                # if vmin < -1.0: vmin = -1.0
                if vmax < 1.0: vmax = 1.1
                Plot_Dict[nuc + dtype]=[[vmin, vmax], r"1 - $\bar I$/$\bar I$$_{0}$"]
            elif dtype == 'Volume':
                Plot_Dict[nuc + dtype]=[[0.0, (np.max(df1.max(skipna=True, numeric_only=True)) * 11.00) / 10.00], r"1 - $\bar V$/$\bar V$$_{0}$"]
        ...
        for dtype in Data_Types:
            ...
            for atom in AtomsList:
                ...
                if Common_Scale.upper() == 'Y':
                    ...
                    dmax = Plot_Dict[atom[0] + dtype][0][1] /1.20
                ...
                cmx_script.write('#save {:}_{:}_{:}.png format png transparentBackground true\n'.format(DataSet,dtype, atom))
                pymol_script.write('\n\n')
                cmx_script.write('\n\n')
                mn+=1
                ln+=1
        cmx_script.write('size stickRadius 0.3\nsize  ballScale 0.4\n')
