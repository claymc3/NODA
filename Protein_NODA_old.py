	...
	if "run" in sys.argv[1].lower():
		...
		for i in range(len(RunParams)):
			if len(RunParams[i]) == 2:
				print(RunParams[i])
				exec(RunParams[i][0] + '= "{:}"'.format(RunParams[i][1]))
			else:
				exec(RunParams[i][0] + '= RunParams[i][1:len(RunParams[i])]')
		MasterList = [x.strip().split(', ') for x in rawInputData[1]]

		if eval('Analysis') == 'CSP':
			outpath = os.path.join(CurDir, eval('OutDir') + '/')
			#cvs_path = os.path.join(outpath, 'CSV_files/')
			if not os.path.exists(outpath):
				os.makedirs(outpath)
			CSP.Plot_CSP_data(RunParams, MasterList, outpath)

		if eval('Analysis') == 'Intensity':
		...
		if eval('Analysis') == 'Titration_Ex_hill':
			outpath = os.path.join(CurDir, OutDir + '/')
			if not os.path.exists(outpath):
				os.makedirs(outpath)
			ConcentrationsList = []
