...
##  content to render and image                                                                     ##
##
## Need to identify resonances that were not oberbserved in the apo state (not including Pro for NH) and color them gray
## Need to identify resonances that were not found again in the perturbrd sameple, and color them purple
...
####----------------------------------------------------------------------------------------------####
    ...
    for DataSet in Plot_DataSetList:
        ...
        ln=1
        for dtype in Data_Types:
            cmx_script = open("{:}Chimera_Scripts/{:}_{:}.cxc".format(outpath,DataSet,dtype),'w')
            cmx_header = "show nucleic\nhide #{:}: protein|solvent|H target as\nsurface hide\nstyle (protein|nucleic|solvent) & @@draw_mode=0 stick\ncartoon style modeHelix tube sides 20\ngraphics silhouettes tru\ncartoon style radius 1.5\nset bgColor white\n".format(mn)
            df = dataframs['DF_' + dtype + '_plot']
            Cutoff_dict = MasterDict['Cutoff_' + dtype + '_dict']
            AtomsList=df['atom_sort'].unique().tolist()
        for atom in AtomsList:
            ...
            showlist = 'show '
            outline = ''
            refmissing = "color #{:}:".format(mn)
            missingdat = "color #{:}:".format(mn)
            if len(df2.dropna(subset=[DataSet]).index.tolist()) > 0:
                cmx_script = open("{:}Chimera_scrips/{:}_{:}-{:}.cxc".format(DataSet,atoms,dtype),'w')
                cmx_script.write("key rgb({:3.0f},{:3.0f},{:3.0f}):0.0 rgb({:3.0f},{:3.0f},{:3.0f}):1.0 rgb({:3.0f},{:3.0f},{:3.0f}):2.0  rgb({:3.0f},{:3.0f},{:3.0f}):3.0  rgb({:3.0f},{:3.0f},{:3.0f}):4.0\nkey colorTreatment blended\n".format(255*cmap(0.0)[0],255*cmap(0.0)[1],255*cmap(0.0)[2],255*cmap(0.25)[0],255*cmap(0.25)[1],255*cmap(0.25)[2],255*cmap(0.5)[0],255*cmap(0.5)[1],255*cmap(0.5)[2],255*cmap(0.75)[0],255*cmap(0.75)[1],255*cmap(0.75)[2],255*cmap(1.0)[0],255*cmap(1.0)[1],255*cmap(1.0)[2]))
                cmx_script.write(Key_text[atom[0]+dtype])
                cmx_script.write('open {:} maxModels 1\nhide #3 target a\n'.format(PDB))
                cmx_script.write('rename #3 {:}_{:}_{:}\ncolor #{:} gray(150)\n'.format(DataSet,dtype,atom,mn))
                cmx_script.write("show nucleic\nhide #3: protein|solvent|H target as\nsurface hide\nstyle (protein|nucleic|solvent) & @@draw_mode=0 stick\ncartoon style modeHelix tube sides 20\ngraphics silhouettes tru\ncartoon style radius 1.5\nset bgColor white\n")
                for row in df2.index.tolist():
                    if row in df2.dropna(subset=[References[DataSet]]).index.tolist():
                        missingdat = missingdat + "{:},".format(df2.loc[row,'resid'][1:])
                        used.append(row)
                    if row in df2.dropna(subset=[DataSet]).index.tolist():
                        norm = (df.loc[row,DataSet]-vmin)/(vmax-vmin)
                        if atom in ['CMe','HMe','CDAro','HDAro', 'CEAro', 'HEAro']:
                            cmx_script.write("color #{:}:{:} rgb({:3.0f},{:3.0f},{:3.0f}) target a; ".format(mn,df2.loc[row,'resid'][1:],255*cmap(norm)[0],255*cmap(norm)[1],255*cmap(norm)[2]))
                        else:
                            cmx_script.write("color #{:}:{:} rgb({:3.0f},{:3.0f},{:3.0f}) target c; ".format(mn,df2.loc[row,'resid'][1:],255*cmap(norm)[0],255*cmap(norm)[1],255*cmap(norm)[2]))
                    if row not in used:
                        if float("{0:.3f}".format(abs(df.loc[row,DataSet]))) >= Bins[-1]:
                            cmx_outline = cmx_outline + df2.loc[row,'resid'][1:] + ","
                            cmxlable = cmxlable + df2.loc[row,'resid'][1:] + ","
